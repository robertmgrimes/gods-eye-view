...
test('production registry is exact, canonical, and rejects incomplete contracts', async () => {
  ...
  assert.equal(
    LAYER_STATE_REGISTRY.find((entry) => entry.id === 'ky-kytc-webcams')?.token,
    '4',
  );
  assert.equal(
    LAYER_STATE_REGISTRY.find((entry) => entry.id === 'ca-cwwp-webcams')?.token,
    '5',
  );
  assert.equal(
    LAYER_STATE_REGISTRY.find((entry) => entry.id === 'al-algo-webcams')?.token,
    '6',
  );
  assert.equal(
    LAYER_STATE_REGISTRY.find((entry) => entry.id === 'webcam-explore')?.token,
    '7',
  );
  assert.equal(
    LAYER_STATE_REGISTRY.find((entry) => entry.id === 'nps-nature-cameras')
      ?.token,
    '8',
  );
