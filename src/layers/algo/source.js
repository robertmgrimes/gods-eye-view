import { algoClientMessage } from './model.js';
...
/**
 * Browser client for the same-origin ALGO proxy. Snapshot hosts stay on the
 * server; this module only speaks /api/algo.
 */
export function createAlgoSource({
    ...
    if (!response.ok) {
      const code =
        ...
        payload?.error === 'disabled'
          ? payload.error
          : 'upstream';
      const error = new Error(algoClientMessage(code));
      error.code = code;
  ...
  return {
    ...
    cameras(query = {}, { signal } = {}) {
      ...
      return get(`/api/algo/webcams?${params}`, signal);
    },
    warm(ids = [], { signal } = {}) {
      const seen = new Set();
      const list = [];
      ...
      return get(`/api/algo/webcams/warm?${params}`, signal);
    },
  };
}
