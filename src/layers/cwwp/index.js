...
const PIN = Cesium.Color.fromCssColorString('#f0a04b');
...
/**
 * Caltrans CWWP pins for the current view. A click opens the still through
 * the same-origin proxy. Snapshot bytes refresh on the feed cadence for the
 * pins currently in view. Live video is not opened.
 */
export function createCwwpWebcamsLayer({ source } = {}) {
  if (typeof source?.cameras !== 'function')
    throw new TypeError('Caltrans CWWP webcams require a cameras source');
  ...
  function cameraIdFromEntity(entity) {
    const id = entity?.id;
    return typeof id === 'string' && id.startsWith('cwwp:')
      ? id.slice(5)
      : null;
  }
  ...
  const layer = {
    ...
    name: 'Caltrans CWWP',
    icon: '📷',
    source: 'Caltrans CWWP',
    updateInterval: 0,
    statsRefreshInterval: 1000,
    init(viewer) {
      if (state.viewer)
        throw new Error('Caltrans CWWP webcam layer is already initialized');
      state.viewer = viewer;
      ...
      state.popover = createCwwpPopover({
